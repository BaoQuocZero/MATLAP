...
% Hi?n th? ?nh g?c và các ?nh t? các bit plane trong cùng m?t c?a s?
figure;
subplot(2, 2, 1); imshow(I);  % Hi?n th? ?nh g?c
title('?nh g?c');

subplot(2, 2, 2); imshow(I3 * 255, []);  % Hi?n th? ?nh i3 (bit plane th? 3)
title('Bit plane th? 3');

subplot(2, 2, 3); imshow(I6 * 255, []);  % Hi?n th? ?nh i6 (bit plane th? 6)
title('Bit plane th? 6');

subplot(2, 2, 4); imshow(i78 * 255, []);  % Hi?n th? ?nh i78 (bit plane th? 7 và 8)
